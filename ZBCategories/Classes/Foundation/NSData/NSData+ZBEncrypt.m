...
@implementation NSData (ZBEncrypt)
/**
 *  利用AES - 加密数据
 *
...
{
    return [self zb_CCCryptData:self algorithm:kCCAlgorithmAES128 operation:kCCEncrypt key:key iv:iv];
}
/**
 *  利用AES - 解密数据
 *
...
{
  return [self zb_CCCryptData:self algorithm:kCCAlgorithmAES128 operation:kCCDecrypt key:key iv:iv];
}
/**
 *  利用DES - 加密数据
 *
...
{
  return [self zb_CCCryptData:self algorithm:kCCAlgorithmDES operation:kCCEncrypt key:key iv:iv];
}
/**
 *  利用DES - 解密数据
 *
...
{
  return [self zb_CCCryptData:self algorithm:kCCAlgorithmDES operation:kCCDecrypt key:key iv:iv];
}


static void zb_fixKeyLengths(CCAlgorithm algorithm, NSMutableData * keyData, NSMutableData * ivData)
{
...
- (NSData *)zb_CCCryptData:(NSData *)data
    ...
    zb_fixKeyLengths(algorithm, keyData,ivData);
...
+ (NSData *)zb_encryptData:(NSData *)data publicKey:(NSString *)pubKey
{
    ...
    return [NSData encryptData:data withKeyRef:keyRef isSign:NO];
}
...
+ (NSData *)zb_encryptData:(NSData *)data privateKey:(NSString *)privKey
{
    ...
    return [NSData encryptData:data withKeyRef:keyRef isSign:YES];;
}
...
+ (NSData *)zb_decryptData:(NSData *)data publicKey:(NSString *)pubKey
{
    ...
    return [NSData decryptData:data withKeyRef:keyRef];
}
...
+ (SecKeyRef)addPrivateKey:(NSString *)key{
    ...
    // This will be base64 encoded, decode it.
    NSData *data = base64_decode(key);
    data = [NSData stripPrivateKeyHeader:data];
    if(!data){
        return nil;
    }
